...
Implemented with synthetic tests; protected-data materialization completed on
Calculco (OAR jobs 830 and 1055, 2026-07-05/06). Review findings:
...
- `pipeline/preprocessing.py` fits train-only imputation, scaling, encoding,
  and categorical vocabulary artifacts for model-ready tabular rows.
- `pipeline/config.py` defines `FEATURES_ROOT`, `TRAINING_ROOT`, `FEATURE_VERSION`, `LABEL_VERSION`, `SPLIT_VERSION`, and `DEFAULT_MODELING_PARAMETERS`, including the default `candidate_token_strategy = rxnorm_or_atc`.
...
- Add `pipeline/features.py` with `FeatureBuildConfig` and `build_feature_artifacts(config)`. CLI: `uv run python -m pipeline.features`.
  `patient_stay_features` uses stay-hash-batched materialization
  (`--stay-feature-batches`) so large lab/vital aggregates can be built within
  Calculco cgroup memory limits.
...
- Add `pipeline/preprocessing.py` with `PreprocessingBuildConfig` and
  `build_preprocessing_artifacts(config)`. CLI:
...
- Write ignored artifacts:
  ...
  - `Dataset/processed/training/preprocessing/train_preprocessing_sample.parquet`
  - `Dataset/processed/training/preprocessing/train_fitted_preprocessor.joblib`
...
7. **P2 candidate catalog**
   Build condition-specific candidate catalogs from MIMIC train positives only. Use `index_condition_token = COALESCE(project_condition_token, normalized_condition_token)`. Use default canonical medication token `rxnorm:{rxcui}` when available, else `atc:{ATC3}` from the harmonized medication mapping. Exclude unmapped condition/medication rows from candidates and report aggregate coverage loss.
   For coverage-sensitivity and external-overlap reruns, use
   `--candidate-token-strategy atc3_or_rxnorm` to prefer ATC-3 class tokens
   before RxNorm ingredient tokens while still fitting candidates from MIMIC
...
9. **P2 train-fitted preprocessing artifact**
   Fit imputation, scaling, encoding, and categorical vocabularies on MIMIC
   train rows only, using the Milestone 6 ranking table joined to stay
   features. Save fitted transform state under ignored
   `Dataset/processed/training/preprocessing/`; report only aggregate counts
   and category cardinalities in `reports/preprocessing_manifest.json`.

10. **P3 documentation and workflow updates**
